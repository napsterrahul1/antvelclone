...
app.controller('AutoCompleteCtrl', function($scope,$http,$location) {
	...
	$scope.selectedItem = function(result){
		...
		if (result.kind === 'suggestions') {
			...
			window.location = base_url +'/products/'+result.originalObject.id;
			...
			$location.path(base_url +'/products/');
...
app.controller('listGroupCtrl', function($scope, $http, notify) {
	...
	$scope.deleteFromGroup = function(id){
	   ...
	   $http.delete(base_url +'productsGroup/'+id, {ignoreLoadingBar : true})
	   		.success(function(data){
...
['$scope','$http','$timeout','Templates',function($scope,$http,$timeout,Templates){
	...
	var repeat=function(){
			...
			loop=true;
			ajax=$http.get(
				base_url + '/user/notices'+($scope.date===undefined?'':'?date='+$scope.date), {ignoreLoadingBar: true}
			).success(function(data){
		...
		check=function(notice){
			...
			$scope.push=0;
			$http.get(
				base_url+'/user/notices/check/'+(id||'')+'?date='+$scope.date,
				{ignoreLoadingBar: true}
			).success(function(data){
            ...
            $http.post('/wishes/store', $scope.newList).
                success(function(data, status) {
                    if (data.success) {
                        ...
                        if(productId){
                        	window.location.replace(base_url +'/products/'+productId);
                        }else{
                        	window.location.replace(base_url +'/wishes/directory');
                        }
                    }else{
...
app.controller('AddressesControllerModal', function($scope, $http, $rootScope, $uibModalInstance, notify, $window, PassInfo){
	...
	{
		$http.put(base_url + '/user/address/store', $scope._address).
			success(function(data, status)
			{
				if (data.success)
				{
	...
	$scope.update = function(){
		...
		$http.put(base_url + '/user/address/'+$scope._address.id, $scope._address).
...
app.controller('getDetailsProductInCart',function($scope,$http,data){
    ...
    var changeKey=function(email,action){
    	...
    	if (obj){
	    	$http.post(base_url +'/editKeyVirtualProductsOrders/'+data.data,obj).success(function(data) {
	    		if (data.message){
		            $scope.message=data.message;
		    		$scope.show=true;
		        }else{
