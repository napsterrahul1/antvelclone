...
(function (root, factory) {
  ...
  angular.module('angucomplete-alt', [] )
    ...
    $templateCache.put(TEMPLATE_URL,
        ...
        '        <img ng-if="result.image && result.image != \'\'" ng-src="'+ base_url +'/[[result.image]]" class="angucomplete-image"/>' +
        '        <div ng-if="!result.image && result.image != \'\'" class="angucomplete-image-default"></div>' +
        '      </div>' +
        '      <div class="angucomplete-title" ng-if="matchClass" ng-bind-html="result.title+result.divider"></div>' +
      ...
      link: function(scope, elem, attrs, ctrl) {
        ...
        function findMatchString(target, str) {
          ...
          if (matches) {
            result = target.replace(re,
              ...
              var lettersStr=str.replace(/ /g,'').split("");
              result=target;
              for (var i in lettersStr){
                    result= result.replace(new RegExp(lettersStr[i], 'gi'), '[[$&]]');
              }
              result=result.replace(/\[\[/g,'<span class="'+ scope.matchClass +'">').replace(/\]\]/g,'</span>');
          }
          return result;
        }
        ...
        function keydownHandler(event) {
          ...
          if (which === KEY_EN && scope.results) {
            if (scope.currentIndex >= 0 && scope.currentIndex < Object.size(scope.results)) {
              event.preventDefault();
            ...
            event.preventDefault();
            if ((scope.currentIndex + 1) < Object.size(scope.results) && scope.showDropdown) {
              scope.$apply(function() {
                scope.currentIndex ++;
                updateInputField();
              });
            ...
            if (scope.results && Object.size(scope.results) > 0 && scope.showDropdown) {
        ...
        function getRemoteResults(str) {
          var params = {ignoreLoadingBar:true},
        ...
        function processResults(responseData, str){
          ...
          if (typeof responseData === 'object') {
            ...
            scope.results = procesingResults(responseData, str);
          }
        }
          ...
          if (responseData ) { 
            ...
            //for (i = 0; i < responseData.length; i++) {
              if (scope.titleField && scope.titleField !== '') {
                text =  extractTitle(responseData);
              ...
              if (scope.descriptionField) {
                description =  extractValue(responseData, scope.descriptionField);
              }
