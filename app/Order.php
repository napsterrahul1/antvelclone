...
class Order extends Model
{
     ...
     * @return Response
     */
    public static function placeOrders($type_order,$request)
    {
        ///////transaction information stored in  Transaction////
        if($request != 'cod'){
            $transaction = Transaction::Success($request);
            $txnid = $request->txnid;
        }

        $cart = self::ofType($type_order)->auth()->whereStatus('open')->orderBy('id', 'desc')->first();
        $cart->pay_mode = $request='cod' ? 'cod' : 'online';
        $cart->save();
        ...
        if ($pointsModified) {
            ...
            foreach ($seller_email as $email) {
                ...
                $data = [
                    'orderId' => $newOrder->id,
                    'order'   => $mailed_order,
                    'txnid'   => $request!= 'cod' ? $request->txnid : 'null',
                ];

                $title = trans('email.new_order_for_user.subject')." (#$newOrder->id)";

                $mail = Mail::queue('emails.neworder', compact('data', 'title'), function ($message) use ($user) {
                ...
                $mail1 =Mail::queue('emails.sellerorder', compact('data', 'title'), function ($message) use ($email) {
    ...
    public static function emailbuyer($uid,$order_track_status){
...
if($user){
    ...
    $data = [
        'order_id'      => $user->id,
        'email'         => $user->email,
        'email_message' => 'your Product is'.$order_track_status,
        'new_status'    => 'status',
