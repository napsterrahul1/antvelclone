...
//        $this->middleware('auth');
     ...
     */
    public static function showOderDetail($orderid)
    {
        $panel = [
        ...
        if (($user->role == 'admin') || ($user->role == 'subadmin')) {
            ...
            if ($order) {
                ...
                if ($order->order_track_status == 'initiated') {
                    $order['order_track_status_id'] = 1;
                } elseif ($order->order_track_status == 'packed') {
                    $order['order_track_status_id'] = 2;
                } elseif ($order->order_track_status == 'dispatched') {
                    $order['order_track_status_id'] = 3;
                } elseif ($order->order_track_status == 'delivered') {
                    $order['order_track_status_id'] = 4;
                }
            }
            return view('orders.trackOrderDetail', compact('user', 'panel', 'orderAddress', 'is_buyer', 'order', 'orderAddress', 'order_comments', 'totalItems', 'grandTotal'));
            ...
            if ($order) {
                ...
                $grandTotal = $order->details->sum('price');
                $order['order_track_status_id'] = Utility::getOrderTrackSatusIdByStatusName($order->order_track_status);
                return view('orders.trackOrderDetail', compact('user', 'panel', 'orderAddress', 'is_buyer', 'order', 'orderAddress', 'order_comments', 'totalItems', 'grandTotal'));
            }
        }
    }
    ...
    public function checkOutResume($addressId)
    {
        ...
        if ($userAddress) {
            ...
            if ($user->current_points < $total_points && config('app.payment_method') == 'Points') {
                ...
                $hash = '';
                // Hash Sequence
                $hashSequence = $merchantkey.'|'.$txnid.'|'.$amount.'|fastrack|'.$user->firstname.'|'.$user->email.'||||||||||';
                $hashSequence .= '|'.$SALT;
                $hash = strtolower(hash('sha512', $hashSequence));


                return view('orders.cart', compact('merchantkey','hash','txnid','cart', 'user', 'panel', 'isResume', 'cartAddress', 'totalItems', 'totalAmount'));
            }
        } else {
    ...
    public function placeOrder($type)
    {
        ...
        $errors = Order::placeOrders($type,$type='cod');
        if ($errors) {
    ...
    public function success(Request $request)
    {
        if($request->status == 'success'){
            $type = 'cart';
            $errors = Order::placeOrders($type,$request);
            if ($errors) {
                return redirect()->route('orders.show_cart')->withErrors(['main_error' => [$errors]]);
            } else {
                Session::push('message', trans('store.order_placed'));
    ...
    {
            Session::push('message', 'Transaction failed');
            return redirect('user/orders/checkOut');
    }

    public function cancelorder(Request $request)
    {
        Session::push('message', 'Transaction canceled');
        return redirect('user/orders/checkOut');
    }
...
//            ->where('seller_id', $user->id)
    ...
    public function trackOrder() {
        $user = \Auth::user();
        if(!$user) {
            Session::push('message', 'Please login for tracking of product !!');
            return redirect()->back();
        }
        $user = User::findOrFail(\Auth::id())->relationsToArray();
        $panel = $this->view_panel;
        return view('orders.trackOrder', compact('panel','user'));
    }
    public static function trackOrderPost(Request $request)
    {
         $orderid = $request->get('orderid');
        $orderid = str_replace('#','',$orderid);
        return self::showOderDetail($orderid);
    }

    public static function changeOrderTrackSatus($orderid, $ordertrack_statusid) {
        $order_track_status = Utility::getStatusNameByOrderTrackSatusId($ordertrack_statusid);
        $order = Order::find($orderid);
        $order->order_track_status = $order_track_status;
        $order->save();

        $email = Order::emailbuyer($order->user_id,$order_track_status);

        Session::push('message', 'Order Status is Changed Successfully !!');
        return self::showOderDetail($orderid);
    }
    private $view_panel = [
        'left'   => ['width' => '2', 'class' => 'user-panel'],
