...
//                         ->withErrors(array('induced_error'=>array(storage_path().'files/key_code'.$request->input('key'))))->withInput();
        ...
        $name = $admin ? $admin->nickname : '';
        $data['email'] = $admin ? $admin->email : 'talegaon.in@gmail.com';
        $data['message'] = str_replace('[name]', $name, trans('user.product_new_seller'));
        Session::put('data',$data);
        Mail::send('emails.sendMailToAdminAfterProductAdd',$data, function ($message) use ($data) {
            $message->to($data['email'])->subject('New Product is Created');
    ...
    public function show($id)
    {
        ...
        $product = Product::select([
            ...
            'rate_count', 'low_stock', 'status', 'type', 'tags', 'products_group', 'brand','product_status',
        ])->with([
            'group' => function ($query) {
                $query->select(['id', 'products_group', 'features','product_status']);
            },
        ...
        if ($product) {
            ...
            return view('products.detailProd', compact('kind_of_request','product', 'panel', 'allWishes', 'reviews', 'freeproductId', 'features', 'suggestions','user'));
        } else {
...
//        $basepath = $basepath . 'storage/files';
        $path = File::section('product_img')->upload($request->file('file'));
        return $path;
    }
    ...
    private function validateFeatures($data)
    {
       ...
       // dd($data, $features_rules,$message_rules);
        ...
        foreach ($features as $row) {
            $values = [];
            if (($row['max_num_values'] * 1) !== 1) {
                for ($i = 1; $i <= (($row['max_num_values']-1) * 1); $i++) {
    ...
    public static function setCounters($product = null, $data = [], $wrapper = '')
    {
        if (\Auth::user() && $product != '' && count($data) > 0) {
            ...
            if (count(array($_array)) == 0 || !in_array($product->id, array($_array))) {
     ...
     */
    public static function getSuggestions($data,$catid =1)
    {
        $options = [
            'user_id'         => '',
            'preferences_key' => '',
        ...
        if (is_array($suggest_listed) ? count($suggest_listed) : 0) {
        ...
        if (is_array($data['preferences_key'])) {
            foreach ($data['preferences_key'] as $id) {
                $needleAux = Product::select('tags', 'name','product_status')
                    ->where('id', $id)
...
//        }
     ...
     */
    public static function getRandCategoryId($catid =1)
    {
...
//                dd($response);
     ...
     */
    public static function getTopRated($point = '5', $limit = 5, $tags = false,$cid = 1)
    {
//        dd($cid);
        if ($tags == true) {
