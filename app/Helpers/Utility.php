...
class Utility
{
    ...
    public static function showPrice($arg)
    {
        ...
        $options['amount'] = self::addCommission($options['amount']);
...
//            return $dns1d->getBarcodeSVG(trim($product->bar_code) != '' ? $product->bar_code : $product->id, $type);
    ...
    public static function addCommission($amount) {
        $commission = 0;
        if($amount) {
            if($amount <= 5000) {
                $commission = (6/100)*$amount;
            }elseif($amount > 5000 && $amount <= 10000) {
                $commission = (8/100)*$amount;
            }elseif ($amount > 10000) {
                $commission = (6/100)*$amount;
            }    
        }
    ...
    public static function getOrderTrackSatusIdByStatusName($order_track_status) {
        $status_id = 0;
        if ($order_track_status == 'initiated') {
            $status_id = 1;
        }elseif ($order_track_status == 'packed') {
            $status_id = 2;
        }elseif ($order_track_status == 'dispatched') {
            $status_id = 3;
        }elseif ($order_track_status == 'delivered') {
            $status_id = 4;
        }
        return $status_id;
    }
    public static function getStatusNameByOrderTrackSatusId($order_track_status_id) {
        $status_name = '';
        if ($order_track_status_id == 1) {
            $status_name = 'initiated';
        }elseif ($order_track_status_id == 2) {
            $status_name = 'packed';
        }elseif ($order_track_status_id == 3) {
            $status_name = 'dispatched';
        }elseif ($order_track_status_id == 4) {
