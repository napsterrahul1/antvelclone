...
@forelse($elements as $cat)
    <li class="list-group-item" ng-show="(!search||(('{{ strtolower($cat->name) }}').indexOf(search.toLowerCase())>=0))">
        <div class="row">
            ...
            <div class="col-md-3"><span class="label label-default visible-xs-inline">{{ trans('product.inputs_view.name') }}:</span>  <a href="{!! url('wpanel/category/'.$cat->id) !!}">{{ $cat->name }}</a></div>
            <div class="col-md-2">
            ...
            <div class="col-md-2"><span class="label label-default visible-xs-inline">{{ trans('store.father') }}:</span>
                @if($cat->hasParent())
                    <a href="{!! url('wpanel/category/'.$cat->id) !!}">{{ $cat->parent ? $cat->parent->name : '' }}</a>
                @endif
             </div>
            <div class="col-md-2"><span class="label label-default visible-xs-inline">{{ trans('globals.type') }}:</span> {{ $cat->type }}</div>
            <div class="col-md-2"><a href="{{ url('wpanel/category/'.$cat->id.'/edit') }}">Edit</a></div>
        </div>
    </li>
    @if($cat->hasChildren())
