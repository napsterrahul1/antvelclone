	...
	@section('center_content')
        ...
        <div class="row">
            <div class="col-md-12 text-right">
        		...
        		<button ng-controller="ModalCtrl" ng-click="modalOpen({templateUrl:  '{!! url('/') !!}/user/address/create',controller:'AddressesControllerModal', size: 'md'})" class="btn  btn-sm btn-info"><span class="glyphicon glyphicon-plus"></span>&nbsp;{{ trans('address.add') }}</button>

				@if(isset($defaultId) && $defaultId != '')
                	<a class="btn btn-success btn-sm" href="{{ url('user/orders/checkOut/address/'.$defaultId) }}">
					...
					<div class="alert alert-warning">
						...
						<div class="row">
							<div class="col-lg-12">
								...
								<a href="javascript:void(0)" ng-controller="ModalCtrl" ng-click="modalOpen({templateUrl: url('/') + 'user/address/create',controller:'AddressesControllerModal',resolve:'address', size: 'md'})">{{ trans('globals.click_here') }}</a>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="col-lg-4">
						...
						<div class="panel @if ($address->default == '1') panel-success @else panel-primary @endif">
							...
							<div class="panel-footer" ng-controller = "AddressesController">
								...
								<button ng-controller="ModalCtrl" ng-click="modalOpen({templateUrl:'{{ url('/user/address/'.$address->id.'/edit') }}', size: 'md', controller:'AddressesControllerModal' })" class="btn btn-success btn-sm">
									<span class="glyphicon glyphicon-edit"></span>&nbsp;
									{{ trans('globals.edit') }}
								</button>

								@if(isset($defaultId) && $defaultId != '')
                                    <a class="btn btn-primary btn-sm" href="{{ url('user/orders/checkOut/address/'.$address->id)  }}">
										<span class="glyphicon glyphicon-pushpin"></span>&nbsp;
    ...
    <script>
         (function(app){
			...
			app.controller('AddressesController',['$scope','$http', '$window', 'PassInfo', function($scope, $http, $window, PassInfo){
				...
				{
					$http.post(base_url + '/user/address/default?id='+id).
				...
				{
		  			$http.post(base_url + '/user/address/delete', { 'id': id }).
