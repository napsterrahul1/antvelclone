...
@if(isset($features) && is_array($features))
	@foreach($features as $feature)
		...
		<div class="form-group" ng-show="(typeItem=='{{ $feature['type_products'] }}'||'{{ $feature['type_products'] }}'=='all')">
			...
			@for($i=1;$i<= ($feature['max_num_values']-1);$i++)
				<?php
				...
				@if($feature['input_type']=='image')
					<div ng-controller="upload" ng-init="file='{{ $file }}'">
						...
						$path = str_replace('/public',"" ,$path) . '/storage/files';
						?>
	                    <div class="image thumbnail col-sm-2"  style="background-image: url('{!! $path . '[[file]]' !!}');background-size: 100%; min-height: 150px; margin-right: 5px;" ng-click="type_file='{{ isset($urlImage)?$urlImage:'' }}'" ng-class="successClass">
	            ...
	            @elseif($feature['input_type']=='document')
					<div ng-controller="upload" ng-init="file='{{ url($file) }}'">
