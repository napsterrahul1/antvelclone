                ...
                <div class="col-md-12">
                    <div class="btn-group pull-right">
                        <button ng-controller="ModalCtrl"
                                ng-click="modalOpen({templateUrl:'{{ url("productsGroup/".$product["id"]."/edit") }}',controller:'ModalCtrl', noCache:true})" class="btn btn-sm btn-info">
...
{{--                            {!! Form::text('bar_code',null,['class'=>'form-control']) !!}--}}
                    ...
                    <div class="form-group ng-cloak">
                        ...
                        <div class="col-sm-4">&nbsp;
                            ...
                            @if ($product->rate_count > 0)
                                <small>{{ \App\Helpers\Utility::showRate($product->rate_val) }} </small>
                            @endif
                            @endif
                        </div>
                    </div>
                    ...
                    {{--<div class="form-group ng-cloak" ng-show="typeItem=='key'">--}}
                        ...
                        {{--</div>--}}
                        {{--<div class="col-sm-3">--}}
                            {{--<a href="{!! url('products/downloadExample') !!}" class="form-control" target="_blank">{{ trans('product.inputs_view.download_example') }}</a>--}}
                        {{--</div>--}}
                        {{--@if($product)--}}
            ...
            app.controller('upload', ['$scope', '$upload', '$timeout','$http', function ($scope, $upload, $timeout, $http) {
                ...
                $scope.upload = function (files) {
                    if (files && files.length && ($scope.type_file==''||$scope.type_file=='key'||$scope.type_file=='software')) {
                        for (var i = 0; i < files.length; i++) {
                            var file = files[i];
                            var url=base_url +'/products/upload'+($scope.type_file!=''?'_'+$scope.type_file:'');
                            $upload.upload({
                                url: url,
                ...
                $scope.delete = function (old) {
                    ...
                    if (file) {
                        ...
                        $http.post(base_url + '/products/delete_img',{'file':file}).
